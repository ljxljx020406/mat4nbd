...
class Decoder(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, n_block, n_embd, n_head, n_agent,
        ...
        '''
        dec_actor: 解码器中的策略网络，基于rep生成最终的动作。True表示直接生成动作，不需要复杂解码步骤逐步生成动作。默认False
        share_actor: 多个智能体是否共享同一策略。默认False
        '''
...
class MultiAgentTransformer(nn.Module):
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False):
        '''
        生成当前状态下智能体的动作
        available_actions：指定agent实际可选的动作，=0表示不可选位置。[batch_size, num_agents, action_dim]
        deterministic：True表示智能体选择概率最大的动作，False表示根据概率随机选择动作
        '''
        # state unused 改！
