...
def one_link_transmission(distance, channels, Power, frequencies):
    ...
    tmpPower = tmpPower.reshape(80, 1)
...
def _get_node_pair(topology):
    ...
    node_request_probabilities = 1 / node_num * np.ones(node_num, dtype=float)
...
def naive_RWA(topology, service:Service, service_dict, channels, frequencies):
    '''
...
def first_fit(topology, service:Service, service_dict, channels, frequencies):
    '''
    input: topology, service
    output: path_node_list, wavelength j
    '''
...
# 4. release a service
def release_service(topology, service:Service, service_dict, channels, frequencies):
    '''
