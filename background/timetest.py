...
service_dict = {}
row = 0
while services_processed_since_reset < 200:
    ...
    if path != None:
        ...
        print(reason)
        row += 1
        df1 = pandas.DataFrame(reason)
        with pandas.ExcelWriter("阻塞原因测试.xlsx") as writer:
            # 将第一批数据写入工作表
            df1.to_excel(writer, index=False, sheet_name='Sheet1', startrow=row)
...
# 保存topology对象到文件
with open('../topology/usnet_600services.pkl', 'wb') as f:
    pickle.dump(topology, f)

with open('../service/usnet_600service_dict.pkl', 'wb') as f:
